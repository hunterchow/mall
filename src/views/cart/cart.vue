<template>
  <div>
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascript:;" @click="cartEdit(' ',item)" class="checkbox-btn item-check-btn"
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="cartEdit('dec',item)">-</a>
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascript:;" @click="toggleSelectAll">
                  <span class="checkbox-btn item-check-btn" :class="{'checked':selectAllFlag}">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{subTotal | currency('￥')}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':!isSelect}" @click="checkOut">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal :mdShow="mdShow_cart" v-on:close="closeModal">
      <p slot="message">确认要删除此条数据?</p>
      <div slot="btnGroup">
        <a href='javascript:;' class="btn btn--m" @click="cartDel">确认</a>
        <a href='javascript:;' class="btn btn--m" @click="mdShow_cart=false">取消</a>
      </div>
    </modal>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isSelect(){
        return this.cartList.some((item) => item.checked == '1');
      }
    },
    methods: {
      init(){
      ...
      showDelModal(item){
        this.mdShow_cart = true;
        this.delItem = item;
      },
      cartDel(){
      	var productNum = this.delItem.productNum;
        axios.post('/users/cartDel', {'productId': this.delItem.productId}).then((response) => {
          let res = response.data;
          if (res.status == '0') {
            this.mdShow_cart = false;
            this.init();
      ...
      toggleSelectAll(){
        ...
        axios.post('/users/cartSelectAll', {'selectAll': flag}).then((response) => {
          let res = response.data;
          if (res.status == '0') {
            this.init();
          }
