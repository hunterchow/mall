<template>
  <div>
              ...
              <ul>
                <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex == index}"
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="showAddressDel(item.addressId)">
    ...
    <modal :mdShow="mdShow_address" v-on:close="closeModal">
      <p slot="message">确认要删除此条数据?</p>
      <div slot="btnGroup">
        <a href='javascript:;' class="btn btn--m" @click="addressDel">确认</a>
        <a href='javascript:;' class="btn btn--m" @click="mdShow_address=false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="mdShow_address1" v-on:close="closeModal">
      <p slot="message">地址列表至少要有一条数据，已无法继续删除！</p>
      <div slot="btnGroup">
        <a href='javascript:;' class="btn btn--m" @click="mdShow_address1=false">好的</a>
      </div>
    </modal>
  ...
  export default {
    ...
    data(){
      return {
        ...
        limit: 3,
        checkIndex: 0,
        mdShow_address: false,
        mdShow_address1: false,
        addressId: '',
        selectedAddrId: ''
      }
    },
    ...
    methods: {
      ...
      showAddressDel(addressId){
        if (this.addressList.length > 1) {
          this.mdShow_address = true;
          this.addressId = addressId;
        } else {
          this.mdShow_address1 = true;
        }
      },
      closeModal(){
        this.mdShow_address = false;
        this.mdShow_address1 = false;
      },
      addressDel(){
        axios.post('/users/addressDel', {'addressId': this.addressId}).then((response) => {
          let res = response.data;
          if (res.status == '0') {
            this.mdShow_address = false;
            this.getAddress();
